    ...
    def _process_job(self, job: ProcessingJob) -> bool:
        ...
        try:
            ...
            if not violations_to_process:
                logger.info(
                    f"No violations provided in job, fetching from database for session {job.session_id}"
                )
                violations_to_process = self._fetch_violations_from_db(job.session_id)

            logger.info(
                f"Processing {len(violations_to_process)} violations for session {job.session_id}"
            )
            ...
            for violation in violations_to_process:
                try:
                    ...
                    # Check if we already have an enhanced explanation in VKG database first
                    signature = create_violation_signature(violation_obj)

                    # First check in-memory cache for efficiency
                    if str(signature) in explanation_cache:
                        logger.debug(
                            f"Explanation already cached in memory for violation {signature}"
                        )
                        continue

                    # Then check VKG database for existing explanations
                    try:
                        logger.info(f"Loading VKG to check for cached explanation for signature: {signature}")
                        vkg = load_vkg_from_virtuoso()
                        if vkg:
                            logger.info(f"VKG loaded successfully, checking for explanation signature: {signature}")
                            cached_explanation = vkg.get_explanation(signature)
                            if cached_explanation:
                                logger.info(
                                    f"✅ FOUND cached explanation in VKG database for violation {signature} - skipping LLM call!"
                                )
                                # Cache it in memory for faster access
                                explanation_cache[str(signature)] = {
                                    "explanation_natural_language": getattr(cached_explanation, "natural_language_explanation", ""),
                                    "suggestion_natural_language": getattr(cached_explanation, "correction_suggestions", [""])[0] if getattr(cached_explanation, "correction_suggestions", []) else "",
                                    "proposed_repair": {"query": getattr(cached_explanation, "proposed_repair_query", "")},
                                    "has_enhanced": True,
                                    "from_vkg": True
                                }
                                continue
                            else:
                                logger.warning(f"❌ No cached explanation found in VKG for signature: {signature}")
                        else:
                            logger.error("❌ VKG could not be loaded from database")
                    except Exception as e:
                        logger.error(f"❌ Error checking VKG database for existing explanation: {e}")
                        import traceback
                        logger.error(f"Traceback: {traceback.format_exc()}")
                        # Continue with LLM call if VKG check fails
                    ...
                    explanation = generate_enhanced_explanation(violation_obj)

                    if explanation:
                        logger.info(f"Successfully generated enhanced explanation for violation in session {job.session_id}")
                        # Cache the explanation using the ConstraintViolation object
                        cache_explanation(violation_obj, explanation)
                        processed_count += 1
                    else:
            ...
            # Persist the VKG to Virtuoso after processing violations
            if processed_count > 0:
                logger.info(f"Persisting VKG to Virtuoso for session {job.session_id}")
                persist_success = persist_vkg_to_virtuoso(vkg)
                if persist_success:
                    logger.info(f"Successfully persisted VKG for session {job.session_id}")
                else:
                    logger.warning(f"Failed to persist VKG for session {job.session_id}")
