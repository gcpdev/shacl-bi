...
def load_vkg_from_virtuoso():
    ...
    try:
        ...
        vkg_graph_uri = getattr(config, 'VIOLATION_KG_GRAPH', 'http://example.org/violation-kg')
        ...
        try:
            ...
            sparql = SPARQLWrapper(getattr(config, 'ENDPOINT_URL', 'http://localhost:8890/sparql'))
            if getattr(config, 'AUTH_REQUIRED', False):
                sparql.setCredentials(getattr(config, 'USERNAME', ''), getattr(config, 'PASSWORD', ''))
        ...
        ontology_path = getattr(config, 'VIOLATION_KG_ONTOLOGY_PATH', 'backend/data/violation_kg.ttl')
...
def generate_enhanced_explanation(violation: ConstraintViolation) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if cached_explanation:
            return {
                'natural_language_explanation': clean_llm_text(cached_explanation.natural_language_explanation),
                'correction_suggestions': [clean_llm_text(s) for s in (cached_explanation.correction_suggestions or [])],
            ...
            if repair_object:
                ...
                return {
                    'natural_language_explanation': clean_llm_text(repair_object.get('explanation_natural_language', '')),
                    'correction_suggestions': [clean_llm_text(repair_object.get('suggestion_natural_language', ''))],
...
def batch_generate_explanations(violations: List[ConstraintViolation]) -> List[Optional[Dict[str, Any]]]:
...
def filter_explanations(explanations: List[Dict[str, Any]],
                       min_confidence: float = 0.0,
                       constraint_type: Optional[str] = None) -> List[Dict[str, Any]]:
    """Filter explanations by confidence and/or constraint type."""
    filtered = explanations

    if min_confidence > 0.0:
        filtered = [exp for exp in filtered if exp.get('confidence', 0) >= min_confidence]

    if constraint_type:
        filtered = [exp for exp in filtered if exp.get('constraint_type') == constraint_type]

    return filtered

def cache_explanation_with_ttl(violation: ConstraintViolation,
