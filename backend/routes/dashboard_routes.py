...
def test_violation_count():
    ...
    try:
        ...
        try:
            from functions import get_number_of_node_shapes, get_number_of_paths_in_shapes_graph
...
def debug_shapes_graph():
    ...
    try:
        ...
        for name, query in [("shapes", shapes_query), ("paths", paths_query), ("components", components_query)]:
            try:
                result = virtuoso_service.execute_sparql_query(query)
                count = int(result.get('results', {}).get('bindings', [{}])[0].get(name + 'Count', {}).get('value', 0))
                results[name] = count
            except Exception as e:
