...
def main_content():
    ...
    main_content_data = get_main_content_data(
        directory_path, shapes_graph_name, validation_report_name
    )
...
def violation_details():
    ...
    return (
        jsonify(
            {
                "error": "Violation details functionality temporarily disabled for debugging"
            }
        ),
        503,
    )
