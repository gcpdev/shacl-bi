...
def map_property_shapes():
    try:
        ...
        result = map_property_shapes_to_node_shapes(
            validation_report_uri, shapes_graph_uri
        )
...
def get_violations_per_constraint_type():
    try:
        ...
        result = get_number_of_violations_per_constraint_type_for_property_shape(
            node_shape
        )
        return (
            jsonify({"nodeShape": node_shape, "violationsPerConstraintType": result}),
            200,
        )
    except Exception as e:
        return jsonify({"error": str(e)}), 400
...
def get_total_constraints_count():
    try:
        ...
        return (
            jsonify(
                {"shapesGraph": shapes_graph_uri, "constraintsPerNodeShape": result}
            ),
            200,
        )
