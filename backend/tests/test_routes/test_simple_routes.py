...
class TestSimpleRoutes:
                ...
                "bindings": [
                    {
                        ...
                        "sourceConstraintComponent": {"value": "http://www.w3.org/ns/shacl#MinCountConstraintComponent"},
                        "value": {"value": ""},
                        "sourceShape": {"value": "http://example.org/shapes/PersonShape"}
                    }
                ]
            }
        }
    ...
    @patch('routes.simple_routes.virtuoso_service')
    def test_get_violations_with_session_id(self, mock_virtuoso, client, sample_session_data):
        """Test violations retrieval with session isolation."""
        mock_virtuoso.execute_sparql_query.return_value = {
            "results": {"bindings": []}
        }

        response = client.get(f'/api/violations?session_id={sample_session_data["session_id"]}')
        assert response.status_code == 200
    ...
    def test_get_violations_database_error(self, mock_virtuoso, client):
        ...
        mock_virtuoso.execute_sparql_query.side_effect = Exception("Database connection failed")
    ...
    def test_generate_explanation_phoenix_success(self, mock_signature, mock_srg, mock_load_vkg, client, sample_violation):
    ...
    @patch('routes.simple_routes.load_vkg_from_virtuoso')
    def test_generate_explanation_phoenix_unavailable(self, mock_load_vkg, client, sample_violation):
    ...
    @pytest.mark.skip(reason="Test failing due to missing _get_constraint_info function")
    @patch('routes.simple_routes._get_constraint_info')
    def test_generate_explanation_basic_patterns(self, mock_get_constraint_info, client, sample_violation):
        ...
        for case in test_cases:
            mock_get_constraint_info.return_value = {'constraint_type': case['constraint_id']}
            ...
            data = json.loads(response.data)
            assert case['expected_explanation'] in data['explanation_natural_language'].lower()
    ...
    def test_get_violation_context_pattern(self, client):
        ...
        with patch('routes.simple_routes.virtuoso_service') as mock_virtuoso:
            ...
            with patch('routes.simple_routes.exrex', Mock(getone=Mock(return_value="Example"))):
    ...
    def test_sparql_query_generation(self):
        ...
        from routes.simple_routes import (
            _generate_mincount_query, _generate_maxcount_query,
            _generate_datatype_query, _generate_pattern_query, _generate_in_query
        )

        # Test MinCount query
        query = _generate_mincount_query('http://example.org/s', 'http://example.org/p', 'session_123')
        assert 'INSERT DATA' in query
        assert '$user_provided_value' in query
        assert 'session_123' in query

        # Test MaxCount query
        query = _generate_maxcount_query('http://example.org/s', 'http://example.org/p', 'old_value', 'session_123')
        assert 'DELETE WHERE' in query
        assert 'old_value' in query

        # Test InConstraint query
        query = _generate_in_query('http://example.org/s', 'http://example.org/p', 'old', 'session_123', 'new')
        assert 'DELETE WHERE' in query
        assert 'INSERT DATA' in query
        assert 'old' in query
        assert 'new' in query
