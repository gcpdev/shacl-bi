...
class TestPhoenixService:
    ...
    def test_create_violation_signature(self, mock_create_signature):
        ...
        mock_violation.constraint_id = "http://www.w3.org/ns/shacl#MinCountConstraintComponent"
    ...
    def test_get_cached_explanation(self, mock_load_vkg):
        ...
        mock_violation = Mock()
        mock_violation.focus_node = "http://example.org/resource1"
        mock_violation.constraint_id = "http://www.w3.org/ns/shacl#PatternConstraintComponent"

        # Mock the signature function to return different values
        with patch('functions.phoenix_service.create_violation_signature') as mock_create_signature:
    ...
    @patch('functions.phoenix_service.load_vkg_from_virtuoso')
    @patch('functions.phoenix_service.create_violation_signature')
    def test_generate_enhanced_explanation(self, mock_create_signature, mock_load_vkg, sample_violation):
        ...
        assert result is not None
        assert result['natural_language_explanation'] == "Enhanced explanation"  # clean_llm_text should preserve normal spaces
        assert len(result['correction_suggestions']) == 2
        assert 'proposed_repair_query' in result

    @patch('functions.phoenix_service.load_vkg_from_virtuoso')
    def test_generate_enhanced_explanation_no_vkg(self, mock_load_vkg, sample_violation):
    ...
    def test_batch_generate_explanations(self, mock_load_vkg):
        ...
        mock_vkg = Mock()
        mock_explanations = [
            Mock(natural_language_explanation="Explanation 1", correction_suggestions=[], proposed_repair_query=""),
            Mock(natural_language_explanation="Explanation 2", correction_suggestions=[], proposed_repair_query=""),
            Mock(natural_language_explanation="Explanation 3", correction_suggestions=[], proposed_repair_query="")
        ]
        ...
        class MockViolation:
            def __init__(self):
                ...
                self.constraint_id = "http://www.w3.org/ns/shacl#PatternConstraintComponent"
        ...
        with patch('functions.phoenix_service.create_violation_signature', side_effect=['sig1', 'sig2', 'sig3']):
    ...
    def test_get_explanation_by_type(self, mock_load_vkg):
        ...
        mock_vkg = Mock()
        mock_explanations = [
            Mock(constraint_type="MinCountConstraintComponent", explanation="Missing value"),
            Mock(constraint_type="MaxCountConstraintComponent", explanation="Too many values")
        ]
        mock_vkg.get_explanations_by_type.return_value = mock_explanations
        mock_load_vkg.return_value = mock_vkg

        explanations = phoenix_service.get_explanation_by_type("MinCountConstraintComponent")
        assert len(explanations) == 2
        assert explanations[0]['constraint_type'] == "MinCountConstraintComponent"  # Function returns dicts, not objects
    ...
    def test_violation_to_dict_conversion(self):
        ...
        assert violation_dict['constraint_id'] == "http://www.w3.org/ns/shacl#PatternConstraintComponent"
    ...
    def test_dict_to_violation_conversion(self):
        ...
        assert violation.constraint_id == 'http://www.w3.org/ns/shacl#PatternConstraintComponent'
