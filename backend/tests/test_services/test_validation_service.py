...
class TestValidationService:
    ...
    def test_validate_data_against_shapes(self, mock_virtuoso, sample_data_ttl, sample_shapes_ttl):
        ...
        mock_virtuoso.execute_sparql_query.return_value = {
            "results": {
                "bindings": [
                    {"violation": {"value": "http://example.org/v1"}, "focusNode": {"value": "http://example.org/resource1"}}
                ]
            }
        }
    ...
    def test_get_validation_report(self, mock_virtuoso):
        ...
        mock_virtuoso.execute_sparql_query.return_value = {
            "results": {
                "bindings": [
                    {"reportUri": {"value": "http://example.org/report/123"}, "conforms": {"value": "false"}, "violationsCount": {"value": "5"}}
                ]
            }
        }
    ...
    def test_create_validation_session(self, mock_virtuoso):
        ...
        mock_virtuoso.execute_sparql_query.return_value = {
            "results": {
                "bindings": [
                    {"sessionId": {"value": "session_456"}, "graphUri": {"value": "http://example.org/validation/session_456"}}
                ]
            }
        }
    ...
    def test_get_validation_statistics(self, mock_virtuoso):
        ...
        mock_virtuoso.execute_sparql_query.return_value = {
            "results": {
                "bindings": [
                    {"totalValidations": {"value": "25"}, "successfulValidations": {"value": "22"}, "failedValidations": {"value": "3"}}
                ]
            }
        }
    ...
    def test_batch_validate(self, mock_validate, mock_virtuoso):
        ...
        datasets = [
            {"data_graph": "http://example.org/data1", "shapes_graph": "http://example.org/shapes"},
            {"data_graph": "http://example.org/data2", "shapes_graph": "http://example.org/shapes"}
        ]

        # Mock get_graph_content to return TTL strings (2 datasets × 2 calls each = 4 calls)
        mock_virtuoso.get_graph_content.side_effect = [
            "data1_ttl", "data1_shapes", "data2_ttl", "data2_shapes"
        ]
    ...
    def test_get_validation_history(self, mock_virtuoso):
        ...
        mock_virtuoso.execute_sparql_query.return_value = {
            "results": {
                "bindings": [
                    {"validationId": {"value": "val_1"}, "timestamp": {"value": "2023-01-01T10:00:00"}, "dataGraph": {"value": "http://example.org/data1"}, "shapesGraph": {"value": "http://example.org/shapes"}, "violationsFound": {"value": "8"}},
                    {"validationId": {"value": "val_2"}, "timestamp": {"value": "2023-01-01T11:00:00"}, "dataGraph": {"value": "http://example.org/data2"}, "shapesGraph": {"value": "http://example.org/shapes"}, "violationsFound": {"value": "2"}}
                ]
            }
        }
    ...
    def test_compare_validations(self, mock_virtuoso):
        ...
        validation1 = {"violations": 8, "conforms": False, "timestamp": "2023-01-01T10:00:00"}
        validation2 = {"violations": 3, "conforms": True, "timestamp": "2023-01-01T11:00:00"}
    ...
    def test_get_validation_progress(self, mock_virtuoso):
        ...
        mock_virtuoso.execute_sparql_query.return_value = {
            "results": {
                "bindings": [
                    {"validationId": {"value": "val_1"}, "totalTriples": {"value": "1000"}, "processedTriples": {"value": "650"}, "status": {"value": "processing"}}
                ]
            }
        }
    ...
    def test_retry_validation(self, mock_virtuoso):
        ...
        mock_virtuoso.execute_sparql_query.return_value = {
            "results": {
                "bindings": [
                    {"newValidationId": {"value": "val_2"}, "status": {"value": "started"}}
                ]
            }
        }
    ...
    def test_export_validation_report(self, mock_virtuoso):
        ...
        mock_virtuoso.execute_sparql_query.return_value = {
            "results": {
                "bindings": [
                    {"report": {"value": "Validation Report Content..."}}
                ]
            }
        }
