...
class TestExtendedShaclValidator:
    ...
    def test_validator_initialization(self, mock_graph):
        ...
        from functions.xpshacl_engine.extended_shacl_validator import (
    ...
    def test_validate_with_violations(self, mock_pyshacl_validate, mock_graph):
        ...
        results_graph.add(
            (violation_node, rdflib.RDF.type, URIRef(sh + "ValidationResult"))
        )
        results_graph.add(
            (violation_node, URIRef(sh + "focusNode"), URIRef(ex + "resource1"))
        )
        results_graph.add(
            (
                violation_node,
                URIRef(sh + "sourceConstraintComponent"),
                URIRef(sh + "MinCountConstraintComponent"),
            )
        )
    ...
    def test_get_violation_type_mapping(self):
        ...
        min_count_type = validator._get_violation_(
            URIRef("http://www.w3.org/ns/shacl#MinCountConstraintComponent")
        )
        assert min_count_type == ViolationType.CARDINALITY

        max_count_type = validator._get_violation_(
            URIRef("http://www.w3.org/ns/shacl#MaxCountConstraintComponent")
        )
        assert max_count_type == ViolationType.CARDINALITY

        # Test unknown constraint component
        unknown_type = validator._get_violation_(
            URIRef("http://example.org/UnknownConstraintComponent")
        )
    ...
    def test_extract_violations_from_graph(self, mock_graph):
        ...
        results_graph.add(
            (violation1, rdflib.RDF.type, URIRef(sh + "ValidationResult"))
        )
        results_graph.add(
            (violation1, URIRef(sh + "focusNode"), URIRef(ex + "resource1"))
        )
        results_graph.add(
            (
                violation1,
        ...
        results_graph.add(
            (violation2, rdflib.RDF.type, URIRef(sh + "ValidationResult"))
        )
        results_graph.add(
            (violation2, URIRef(sh + "focusNode"), URIRef(ex + "resource2"))
        )
        results_graph.add(
            (
                violation2,
                URIRef(sh + "sourceConstraintComponent"),
                URIRef(sh + "PatternConstraintComponent"),
            )
        )
    ...
    def test_pattern_violation_enrichment(self, mock_exrex, mock_graph):
        ...
        results_graph.add(
            (violation, URIRef(sh + "sourceShape"), property1)
        )  # Should point to property shape
    ...
    def test_max_count_violation_enrichment(self, mock_graph):
        ...
        results_graph.add(
            (
                violation,
                URIRef(sh + "sourceConstraintComponent"),
                URIRef(sh + "MaxCountConstraintComponent"),
            )
        )
        results_graph.add((violation, URIRef(sh + "resultPath"), URIRef(ex + "email")))
    ...
    def test_in_constraint_violation_enrichment(self, mock_graph):
        ...
        mock_graph.add(
            (URIRef(ex + "list2"), URIRef(rdf + "first"), Literal("Inactive"))
        )
        ...
        results_graph.add(
            (
                violation,
                URIRef(sh + "sourceConstraintComponent"),
                URIRef(sh + "InConstraintComponent"),
            )
        )
    ...
    def test_validator_with_complex_shapes(self, mock_graph):
        ...
        for i, prop in enumerate(["name", "email", "age"]):
            ...
            if prop == "name":
                mock_graph.add(
                    (property_node, URIRef(sh + "datatype"), URIRef(xsd + "string"))
                )
                ...
                mock_graph.add(
                    (property_node, URIRef(sh + "datatype"), URIRef(xsd + "integer"))
                )
                mock_graph.add((property_node, URIRef(sh + "minInclusive"), Literal(0)))
                mock_graph.add(
                    (property_node, URIRef(sh + "maxInclusive"), Literal(150))
                )
    ...
    def test_validator_performance_logging(self, mock_pyshacl_validate, mock_graph):
        ...
        with patch(
            "functions.xpshacl_engine.extended_shacl_validator.logger"
        ) as mock_logger:
            validator = ExtendedShaclValidator(mock_graph)
            data_graph = Graph()
    ...
    @patch("functions.xpshacl_engine.extended_shacl_validator.validate")
    def test_validator_with_empty_results_graph(
        self, mock_pyshacl_validate, mock_graph
    ):
